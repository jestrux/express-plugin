...
export const STORE_KEY = "VISION-BOARD-ADDON";

export function getValueFromLocalStorage(key, defaultValue = null) {
	let store = window.clientStorage.getItem(STORE_KEY);
	if (store) {
...
export function saveValueToLocalStorage(key, value) {
	...
	store[key] = value;
	window.clientStorage.setItem(STORE_KEY, JSON.stringify(store));
}
...
function useLocalStorageState(key, initialValue) {
	...
	// ... persists the new value to clientStorage.
