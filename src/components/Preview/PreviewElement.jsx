	...
	function renderActionButtons(editOnFocus) {
		return (
			<div class="mt-1 flex items-center ActionButtons absolute bg-white z-10 left-0 bottom-0 p-3 opacity-0 pointer-events-none group-hover:reveal">
				{!editOnFocus && (
					<button
						class="mr-2 flex items-center justify-center w-7 h-7 bg-gray-100 rounded-full"
						onClick={() => editElement(element)}
					>
						<svg class="w-4" fill="#f18f16" viewBox="0 0 24 24">
				...
				<button
					class="flex items-center justify-center w-7 h-7 bg-gray-100 rounded-full"
					onClick={() => removeElement(element)}
				>
					<svg class="w-4" fill="#e04b2a" viewBox="0 0 24 24">
