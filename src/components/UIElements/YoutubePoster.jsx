function YoutubePoster(values) {
	this.values = values;
	this.render = () => YoutubePoster.doRender(this.values, this.images);
}

YoutubePoster.label = "Youtube Poster";
...
YoutubePoster.props = {
	...
	heading: {
		type: "text",
		defaultValue: "YouTube Thumbnail with Tailwind CSS",
	},
	color: {
		type: "radio",
		choices: ["purple", "blue", "red", "green", "yellow", "pink"],
		defaultValue: "yellow",
		renderChoice(color, selected) {
			return `
                <span class="rounded-full bg-${color}-500 inline-block p-0.5" style="width: 30px; height: 30px;">
                    <span class="border-2 border-${
						selected ? "white" : "transparent"
					} rounded-full w-full h-full block"></spa>
                </span>
            `;
		},
	},
	...
	codeScreenshot: {
		type: "image",
		defaultValue:
			"https://images.unsplash.com/photo-1517134191118-9d595e4c8c2b?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=MnwxNjE2NXwwfDF8c2VhcmNofDR8fHZzJTIwY29kZXxlbnwwfHx8fDE2NzcxMzY4MDM&ixlib=rb-4.0.3&q=80&w=600",
	},
	flipScreenshots: {
		type: "boolean",
		defaultValue: true,
...
YoutubePoster.doRender = function (options, images) {
	const width = options.width == "Normal" ? "2/3" : "4/5";
	let screenshots = [options.cameraCapture, options.codeScreenshot];

	if (options.flipScreenshots) screenshots = screenshots.reverse();
	...
	return /*html*/ `
        <div class="relative bg-white overflow-hidden aspect-video">
            <!-- Top-left circle -->
            <div aria-hidden="true" class="absolute -top-16 -left-12 bg-${options.color}-200 rounded-full grid place-items-center"
                style="width: 200px; height: 200px"
            >
            <svg class="ml-3 mt-3" width="64" viewBox="0 0 64 72" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <!-- Right big circle -->
            <div aria-hidden="true" class="absolute bg-${options.color}-200 rounded-full -top-20 -right-56"
                style="width: 512px; height: 512px;"
            ></div>
            <!-- Images -->
            <div class="absolute inset-y-0 flex flex-col justify-center right-8">
                <img src="${screenshots[0]}" alt="YouTuber cheerfully giving a shaka" class="w-64 h-36 object-cover rounded-2xl shadow-2xl transform rotate-3" />
                <img src="${screenshots[1]}" alt="Screenshot of Tailwind CSS code" class="w-64 h-36 object-cover rounded-2xl shadow-2xl transform -rotate-3 translate-x-16 -translate-y-2" />
            </div>
            <!-- Play button -->
            <div class="absolute right-12 inset-y-0 flex items-center">
				<a href="#" class="w-12 h-12 bg-${options.color}-500 rounded-full ring-4 ring-white grid place-items-center hover:bg-${options.color}-400 transition">
